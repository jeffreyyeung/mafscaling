...
package com.vgi.mafscaling.loadcomp;
...
public class LoadCompJY extends ACompCalc {
    @Serial
    private static final long serialVersionUID = 2988105467764335997L;
    private static final Logger logger = Logger.getLogger(LoadCompJY.class);
    ...
    private final ArrayList<Double> rpmArray = new ArrayList<>();
    private final ArrayList<Double> mafvArray = new ArrayList<>();
    ...
    public LoadCompJY(int tabPlacement) {
        super(tabPlacement);
        ...
        for (File file : files) {
            ...
            try {
                ...
                if (logThrottleAngleColIdx >= 0 || logAfLearningColIdx >= 0 || logAfCorrectionColIdx >= 0 || logAfrColIdx >= 0 ||
                    logRpmColIdx >= 0 || logTimeColIdx >=0 || logMafvColIdx >= 0 || logMpColIdx >= 0) {
                    if (displayDialog) {
                ...
                if (resetColumns || logThrottleAngleColIdx < 0 || logAfLearningColIdx < 0 || logAfCorrectionColIdx < 0 || logAfrColIdx < 0 ||
                    logRpmColIdx < 0 || logTimeColIdx < 0 || logMafvColIdx < 0 || logMpColIdx < 0) {
                    ColumnsFiltersSelection selectionWindow = new LCJYColumnsFiltersSelection();
                ...
                try {
                    ...
                    while (line != null) {
                        ...
                        pThrottle = throttle;
                        try {
                            throttle = Double.parseDouble(flds[logThrottleAngleColIdx]);
                            // Calculate dV/dt
                            prevTime = time;
                            if (prevTime == 0)
                            ...
                            if (row > 1 && Math.abs(pThrottle - throttle) > thrtlMaxChange) {
                                ...
                                mp = Double.parseDouble(flds[logMpColIdx]);
                                stft = Double.parseDouble(flds[logAfCorrectionColIdx]);
                                ltft = Double.parseDouble(flds[logAfLearningColIdx]);
                                trims = stft + ltft;
                                if (cruiseValue != -1)
                                    cruise = Double.parseDouble(flds[logCruiseStatusColIdx]);
                                if (afrMin <= afr && afr <= afrMax && rpmMin <= rpm && rpm <= rpmMax && mpMin <= mp && mp <= mpMax && dVdt <= dvDtMax && cruise == cruiseValue) {
                                    removed = false;
                                    Utils.ensureRowCount(row + 1, logDataTable);
                                    logDataTable.setValueAt(time, row, 0);
                                    logDataTable.setValueAt(rpm, row, 1);
                                    logDataTable.setValueAt(stft, row, 2);
                                    logDataTable.setValueAt(ltft, row, 3);
                                    logDataTable.setValueAt(mp, row, 4);
                                    logDataTable.setValueAt(mafv, row, 5);
                                    logDataTable.setValueAt(trims, row, 6);
    ...
    protected boolean processLog() {
        try {
            ...
            for (int i = 0; i < logDataTable.getRowCount(); ++i) {
                xStr = logDataTable.getValueAt(i, 4).toString();
                yStr = logDataTable.getValueAt(i, 1).toString();
                valStr = logDataTable.getValueAt(i, 6).toString();
                ...
                x = xAxisArray.get(Utils.closestValueIndex(Double.parseDouble(xStr), xAxisArray));
                y = yAxisArray.get(Utils.closestValueIndex(Double.parseDouble(yStr), yAxisArray));
                val = Double.parseDouble(valStr);

                yData = xData.computeIfAbsent(x, k -> new HashMap<>());
                data = yData.computeIfAbsent(y, k -> new ArrayList<>());
                data.add(val);
            }
            return true;
        }
    ...
    protected boolean displayData() {
        try {
                        ...
                        if (data == null)
                            ...
                            if (cnt > minCellHitCount) {
                                ...
                                if (isLoadCompInRatio)
                                    newTable.setValueAt(val * Double.parseDouble(origTable.getValueAt(j, i).toString()), j, i);
                                else
                                    newTable.setValueAt(val * (100.0 + Double.parseDouble(origTable.getValueAt(j, i).toString())) - 100.0, j, i);
            ...
            Arrays.fill(colorMatrix[0], Color.LIGHT_GRAY);
